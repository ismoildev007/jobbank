...
if (
    ...
    function fetchSlot(branchId, staffId, serviceId, selectedDate) {
        $("#slot-inut").empty();
        $('.slotLoader-skaliaton').append('<div><div class="skeleton chat-skeleton label-loader mb-2 mt-2"></div> <div class="skeleton chat2-skeleton label-loader"></div> </div>');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if ($.isEmptyObject(response.slot)) {
                    if (
                        response.slot_availability &&
                        response.slot_availability.length > 0
                    ) {
                        $("#slot-inut").append(
                        ...
                        $.each(
                            ...
                            function (index, availableDate) {
                                ...
                                <div class="col-lg-4 col-md-6 mt-2" id="slotDateHover">
                                    <div class="time-item" id="time-item-${index}" onclick="selectsRadioDate('${availableDate}')">
                                        <input type="radio" name="available_date" id="date_${index}" value="${availableDate}" class="slot-date-radio" hidden>
                                        <h6 class="fs-12 fw-medium">${availableDate}</h6>
                                    </div>
                                </div>
                            `;
    ...
    function fetchOnlySlot(serviceId, selectedDate) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                let noSlotMessage = '';
                let futureAvailabilityMessage = '';

                // Determine language-specific messages
                if (response.language_id === 27) {
                    // Uzbek
                    noSlotMessage = 'Belgilangan sana uchun mavjud slotlar yo‘q.';
                    futureAvailabilityMessage = 'Kelajakdagi mavjud sana:';
                } else if (response.language_id === 28) {
                    // Russian
                    noSlotMessage = 'Нет доступных слотов на выбранную дату.';
                    futureAvailabilityMessage = 'Ближайшая доступная дата:';
                } else {
                    // Default to English
                    noSlotMessage = 'No slots available for the given date.';
                    futureAvailabilityMessage = 'Future availability date:';
                }
                ...
                if ($.isEmptyObject(response.slot)) {
                    if (
                        ...
                        $("#slot-input").append(
                            `<div class="col-12 fw-bold mt-3 text-center">
                                <p>${noSlotMessage}</p>
                                <p>${futureAvailabilityMessage} <span class="availability-dates"></span></p>
                            </div>`
                        );
                        $.each(
                            response.slot_availability,
                            function (index, availableDate) {
                                var dateHtml = `
                                <div class="col-lg-4 col-md-6 mt-2" id="slotDateHover">
                                    <div class="time-item" id="time-item-${index}" onclick="selectRadioDate('${availableDate}')">
                                        <input type="radio" name="available_date" id="date_${index}" value="${availableDate}" class="slot-date-radio" hidden>
                                        <h6 class="fs-12 fw-medium">${availableDate}</h6>
                                    </div>
                                </div>
            ...
            success: function (data) {
                ...
                if (data.staffs.length === 0) {
                    ...
                    <div class="col-12 text-center">
                        <p class="text-dark fw-bold mt-5">No staff available at this moment. You can proceed further.</p>
                    </div>
                `);
                    return;
                }
    ...
    function fetchlocstaffSlot(branchId, staffId, addServiceIds = []) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#first_name").val(data.user_info.first_name);
                $("#last_name").val(data.user_info.last_name);
                $("#email").val(data.user_info.email);
    ...
    function fetchPayout(serviceId, addServiceIds = []) {
        $.ajax({
            url: "/get-payout",
            type: "POST",
            data: { service_id: serviceId, addService_ids: addServiceIds },
            headers: {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            },
            success: function (data) {
                ...
                <div class="total-wrap">
                    <div class="mb-2 d-flex align-items-center justify-content-between">
                        <h6 class="fw-medium">${$('#payout').data('sub_total')}</h6>
                        <p class="text-gray-9">${subTotal} ${currecy_details.symbol}</p>
                    </div>
                    <div class="mb-2 d-flex align-items-center justify-content-between">
                        <h6 class="fw-medium">Additional Services Total</h6>
                        <p class="text-gray-9">${addServiceTotal} ${currecy_details.symbol}</p>
                    </div>`;
                ...
                data.tax_used.forEach((tax) => {
                    ...
                    payoutHtml += `
                    <div class="mb-2 d-flex align-items-center justify-content-between">
                        <h6 class="fw-medium">${$('#payout').data('tax')} <span class="text-default fw-normal">(${
                            tax.tax_type
                        } ${parseFloat(tax.tax_rate).toFixed(2)}%)</span></h6>
                        <p class="text-gray-9">${taxAmount} ${currecy_details.symbol}</p>
                    </div>`;
                });
                ...
                payoutHtml += `
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <h6 class="fs-14">${totalAmount} ${currecy_details.symbol}</h6>
                    </div>
                </div>`;
                ...
                if (data.wallet_availabe === "yes") {
                    $("#wallet-meg")
                        ...
                        <div class="card wallet-card">
                            <div class="card-body">
                                <h5 class="card-title">${$('#wallet-meg').data('wallet_balance')}</h5>
                                    <p class="card-text">${$('#wallet-meg').data('your_wallet_balance_is_sufficient')}.</p>
                                    <p class="card-text">${$('#wallet-meg').data('available_balance')}: <strong>$${data.wallet_amount}</strong></p>
                            </div>
                        </div>
                    `
                        )
                    ...
                    $("#wallet-meg")
                        ...
                        <div class="card wallet-card">
                            <div class="card-body">
                                <h5 class="card-title">${$('#wallet-meg').data('wallet_balance')}</h5>
                                    <p class="card-text">${$('#wallet-meg').data('insufficient_wallet_balance')}.</p>
                                    <p class="card-text">${$('#wallet-meg').data('available_balance')}: <strong>$${data.wallet_amount}</strong></p>
                            </div>
                        </div>
                    `
    ...
    $(document).ready(function () {
        ...
        $("#prinfo-btn").on("click", function (event) {
            ...
            // if ($('input[name="first_name"]:checked').length === 0) {
            //     toastr.error("Please select a prinfo before continuing.");
            //     return;
            // }

            if ($("#prinfo-form").valid()) {
                let formDataCollection = {};
    ...
    function fetchPersonal(addServiceIds = []) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#state").val(data.user_info.state);
                $("#postal").val(data.user_info.postal_code);
                $(".slot_time").text(data.formatted_date_times.line2);
                $(".slot_day").text(data.formatted_date_times.line3);
                $(".slot_time_day").text(data.formatted_date_times.line1);
                $(".slotLoader").hide();
            },
            error: function (xhr) {
                const errorMessage =
    ...
    document.getElementById("from_time").addEventListener("input", function () {
        const fromTimeInput = this.value;
        if (fromTimeInput) {
            const [hours, minutes] = fromTimeInput.split(":").map(Number);
            let toHours = hours + 1;

            if (toHours >= 24) {
                toHours = toHours - 24; // Handle overflow to the next day
            }
