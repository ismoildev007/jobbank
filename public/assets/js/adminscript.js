                ...
                if (xhr.responseJSON && xhr.responseJSON.message) {
                    if (Array.isArray(xhr.responseJSON.message)) {
                        xhr.responseJSON.message.forEach(function(error) {
                            errorMessageContainer.append('<div>Login failed due to incorrect username or password. Please try again</div>');
                        });
                    } else {
                        errorMessageContainer.append('<div>Login failed due to incorrect username or password. Please try again</div>');
                    }
                                    ...
                                    `<td>
                                        <li style="list-style: none;">
                                         ${
                                                ...
                                                href="#"
                                                data-id="${val.id}" data-name="${val.name}" data-code="${val.code}" data-status="${val.status}" data-isDefault="${val.is_default}" data-symbol="${val.symbol}">
    ...
    function taxOptionsList() {
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#loader-table').addClass('d-none');
                $(".label-loader, .input-loader").hide();
                $('#tax_option_table, .real-label, .real-input').removeClass('d-none');
            },
            error: function (xhr) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code === "200") {
                    ...
                    $('#loader-table').addClass('d-none');
                    $(".label-loader, .input-loader").hide();
                    $('#testimonialsTable, .real-label, .real-input').removeClass('d-none');
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code === "200") {
                    ...
                    $('#loader-table').addClass('d-none');
                    $(".label-loader, .input-loader").hide();
                    $('#subscriberTable, .real-label, .real-input').removeClass('d-none');
...
if (pageValue === "admin.subscription-package") {
    ...
    function populateSubscriptionTable(Subscription, meta) {
        ...
        $('#loader-table').addClass('d-none');
        $(".label-loader, .input-loader").hide();
        $('#subscription_datatable, .real-label, .real-input').removeClass('d-none');
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code === "200") {
                    ...
                    $('#loader-table').addClass('d-none');
                    $(".label-loader, .input-loader").hide();
                    $('#blogCategoryTable, .real-label, .real-input').removeClass('d-none');
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code === "200") {
                    ...
                    $('#loader-table').addClass('d-none');
                    $(".label-loader, .input-loader").hide();
                    $('#blogPostTable, .real-label, .real-input').removeClass('d-none');
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code === "200") {
                    ...
                    $('#loader-table').addClass('d-none');
                    $(".label-loader, .input-loader").hide();
                    $('#roleTable, .real-label, .real-input').removeClass('d-none');
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code === "200") {
                    ...
                    $(".label-loader, .input-loader").hide();
                    $('#permissionTable, .real-label, .real-input').removeClass('d-none');
...
if (pageValue === 'admin.payment-settings') {
    ...
    $(document).ready(function() {
        async function loadGeneralSettings() {
            ...
            if (response.code === 200) {
                const requiredKeys = [
                    'paypal_status','wallet_status','cod_status','paypal_id', 'paypal_secret','stripe_status','payme_status','razorpay_status','bank_status',
                    'paypal_live','stripe_secret','stripe_key','payme_secret','payme_key','mollie_status','razor_key','razor_secret','branch_code','account_number','bank_name'
                ];
                ...
                filteredSettings.forEach(setting => {
                    ...
                    if (setting.key === 'payme_status' && setting.value==='1') {
                        $('#payme_status_show').prop('checked', true);
                    }
                });
        ...
        $(document).on('click, change', '#payme_status_show', function(e) {
            if($("#payme_status_show").prop('checked') == true){
                var checkedstatus=1;
            }else{
                var checkedstatus=0;
        ...
        $('#paymeSettingForm').on('submit', function(e) {
            e.preventDefault();
            let formData = $(this).serialize();
            $.ajax({
                url: '/api/admin/updatepaymentSettings',
            ...
            list['bookingdata'].forEach(data => {
                ...
                tableBody += `
                    <tr>
                        ...
                        data-id="${data.id}" data-date="${data.bookingdate}" data-status="${data.booking_status_label}"  data-provider="${data?.product?.created_by?.user_details?.first_name ?? ""}" data-amount="${list['currency']}${data.serviceamount}"  data-username="${data?.user?.user_details?.first_name ?? ""}" data-product="${data.source_name}" data-address="${data.user_city}">
...
if(pageValue=='admin.ticket' || pageValue=='staff.tickets'){
    ...
    function applyTicketStatusStyles(){
        $('.ticket-status').each(function (index) {
            ...
            $(this).removeClass('badge ms-2 badge-primary-transparent badge-soft-info badge-soft-warning badge-soft-success status-unknown');
     ...
     $(document).ready(function () {
        ...
        $('#assignticketform').on('submit', function (event) {
            ...
            if(userid!=''){
                ...
                .done((response) => {
                    $("#assignticket").removeAttr("disabled").html($('#ticketList').data('assign'));
                    ...
                    if (response.code=='200') {
                        ...
                        var assignhtml=`<img src="${response.data.assinee_profile_image}" class="avatar avatar-xs rounded-circle me-2" alt="img">
                                                    ${$('#ticketList').data('assignto')} <span class="text-dark ms-1 assigneename">${username}</span>`;
                        assigndata.append(assignhtml);
                        $('#assignBtn'+ id).addClass('d-none');
                        $('.ticketstatus' + id).html(`<i class="ti ti-circle-filled fs-5 me-1"></i>${$('#ticketList').data('assigned')}`);
                        $('.ticketstatus' + id).data('status', 2);
                        $('.updatedTime' + id).html(`<i class="ti ti-calendar-bolt me-1"></i>${$('#ticketList').data('updated') + ' ' + response.data.ticket_updated_at}`);
...
if(pageValue=='admin.ticketdetails' || pageValue=='staff.ticketdetails'){
    ...
    $('.updatestatus').click(function(){
        ...
        if(oldstatus!=status){
            ...
            .done((response) => {
                $(".updatestatus").removeAttr("disabled").html($('.updatestatus').data('update_status_text'));
                if (response.code=='200') {
                    var ticketStatusText = '-';
                    var ticketStatusClass = '';

                    $('.ticketstatus' + ticketid).removeClass('badge badge-primary-transparent badge-soft-info badge-soft-warning badge-soft-success status-unknown');
                    if (status == 1) {
                        ticketStatusText = $('.ticket-details').data('open');
                        ticketStatusClass = 'badge badge-primary-transparent ms-2';
                    } else if (status == 2) {
                        ticketStatusText = $('.ticket-details').data('assigned');
                        ticketStatusClass = 'badge badge-soft-warning ms-2';
                    } else if (status == 3) {
                        ticketStatusText = $('.ticket-details').data('inprogress');
                        ticketStatusClass = 'badge badge-soft-info ms-2';
                    } else if (status == 4) {
                        ticketStatusText = $('.ticket-details').data('closed');
                        ticketStatusClass = 'badge badge-soft-success ms-2';
                    }

                    $('.ticketstatus' + ticketid).html(`<i class="ti ti-circle-filled fs-5 me-1"></i>${ticketStatusText}`);
                    $('.ticketstatus' + ticketid).addClass(ticketStatusClass);
                    $('.updatedTime' + ticketid).html(`<i class="ti ti-calendar-bolt me-1"></i>${$('.ticket-details').data('updated') + ' ' + response.data.ticket_updated_at}`);
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code === 200) {
                    ...
                    $('#loader-table').addClass('d-none');
                    $(".label-loader, .input-loader").hide();
                    $('#staffTable, .real-label, .real-input').removeClass('d-none');
