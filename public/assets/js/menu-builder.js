...
var updateOutput = function (e) {
  ...
  var menuEditor = $("#menuBuilderForm");
  var editButton = $("#updateBtn");
  var currentEditName = $("#currentEditName");
  var currentEditURL = $("#currentEditURL");
  ...
  var prepareEdit = function () {
    ...
    $('#url').val(target.data("url"));
    $('#target').val(target.data("target")).trigger('change');
    $('#method').val('update');
    ...
    currentEditURL.val(target.data('url'));
    editButton.data("owner-id", target.data("id"));

    $('#menuBuilderForm').find('.form-control').removeClass('is-invalid').removeClass('is-valid');
    $('#menuBuilderForm').find('.error-text').text('');
  ...
  var editMenuItem = function () {
    ...
    if (validateName(name, method, currentName) & validateURL(url, method, currentURL)) {
        
        target.data("name", name);
        target.data("url", url);
        target.data("target", hrefTarget);
        
        target.find("> .dd-handle").html(name);
        ...
        menuEditor.find('.form-control').removeClass('is-invalid').removeClass('is-valid');
        $('#menuBuilderForm').find('.error-text').text('');
        ...
        var id = $('#save_all_menus').data('id');

        saveAllMenus(id, jsonData);
  ...
  var addToMenu = function (menu_id = '', name, url, target, page_id, custom ='') {

    if (menu_id == '') {
        menu_id = 'new-' + newIdCount;
    }
    ...
    nestableList.append(`
        <li class="dd-item" 
            ...
            `<span class="button-delete btn btn-danger btn-sm pull-right" data-owner-id="${menu_id}"> 
                <i class="ti ti-trash" aria-hidden="true"></i> 
            </span>` : '' }
            ${ $('#has_permission').data('edit') == 1 ?
            `<span class="button-edit btn btn-info btn-sm pull-right" data-owner-id="${menu_id}">
  ...
  $(function () {
    ...
    $(document).on('click', '#addNewBtn', function() {
        ...
        var method = $('#method').val('add');

        if (validateName(name, method) & validateURL(url, method)) {
            var result = addToMenu(menu_id = '', name, url, target, page_id, custom);
        }
