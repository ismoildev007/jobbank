        ...
        async function loadGeneralSettings() {
            ...
            if (response.code === 200) {
                const requiredKeys = [
                    ...
                    'company_name', 'country', 'state', 'city', 'save_single_vendor_status','fax_no', 'website'
                ];
                ...
                filteredSettings.forEach(setting => {
                    ...
                    if (setting.key === 'save_single_vendor_status') {
                        $("#save_single_vendor_status").val(setting.value);
...
if (pageValue === 'admin.dt-settings') {
    ...
    function loadOtpSettings() {
        $.ajax({
            ...
            success: function(response) {
                if (response.code === 200) {
                    const requiredKeys = ['otp_type', 'date_format_view', 'time_format_view', 'timezone_format_view', 'login'];
                    const filteredSettings = response.data.settings.filter(setting => requiredKeys.includes(setting.key));
...
//product-settings
if (pageValue === 'admin.product-settings'){
    ...
    function loadProductSettings() {
        $.ajax({
            ...
            success: function(response) {
                if (response.code === 200) {
                    const requiredKeys = ['product', 'service'];
                    const filteredSettings = response.data.settings.filter(setting => requiredKeys.includes(setting.key));
    ...
    $('#product_setting_form').submit(function(event) {
        ...
        let formData = new FormData();
        formData.append('group_id', $('#group_id').val());
        formData.append('product', $('#product').is(':checked') ? 1 : 0);
        formData.append('service', $('#service').is(':checked') ? 1 : 0);

        $.ajax({
            url: "/api/admin/update-cookies-setting",
            method: "POST",
            data: formData,
            headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('admin_token'),
            ...
            cache: false,
            beforeSend: function () {
                $('.product_update_btn').attr('disabled', true).html('<div class="spinner-border text-light" role="status"></div>');
            },
        }).done((response) => {
            $('.product_update_btn').removeAttr('disabled').html('Save');
            if (response.code === 200) {
                // toastr.success(response.message);
                if (languageId === 2) {
                    loadJsonFile(response.message, function (langtst) {
...
if (pageValue === 'admin.db-settings') {
    ...
    $('#image_sign1').on('change', function (event) {
        var input = event.target;
        if (input.files && input.files[0]) {
            var reader = new FileReader();

            reader.onload = function (e) {
                $('#invoice_company_logo_image1').attr('src', e.target.result); // Set the image source
            };
    ...
    $(document).ready(function () {
        $('#add_language').on('show.bs.modal', function () {
          //  $('#addLanguageForm')[0].reset(); // Reset the form data
            $(".error-text").text("");
            $(".form-control").removeClass("is-invalid is-valid");
        });
        language_table();
        ...
        $('#searchLanguage').on('input', function () {
            fetchLanguages(1); // Reset to the first page on new search
        });
    });

    function language_table() {
        fetchLanguages(1); // Load the first page by default
    }

    function fetchLanguages(page) {
        $.ajax({
    ...
    function populateLanguageTable(languages, meta) {
        ...
        if (languages.length > 0) {
            languages.forEach(language =>  {
                tableBody += `
                    <tr>
                        <td>truelysell-laravel-${language.id}</td>
                        <td>${language.show_date} ${language.show_time}</td>

                        <td>
                        <li class="d-flex align-items-center" style="list-style: none;">
                        <a href="dbdownload/${language.id}"> <i class="ti ti-cloud-download fs-20 m-2"></i></a>
    ...
    function setupPagination(meta) {
        ...
        $('#pagination').on('click', '.page-link', function (e) {
            ...
            fetchLanguages(page); // Fetch languages for the selected page
        });
    }
    ...
    $(document).ready(function () {
        $('#addLanguageForm').validate({
            ...
            messages: {
                name: {
                    required: $('#language_name_error').data('required'),
                },
                code: {
                    required: $('#language_code_error').data('required'),
                }
            },
            errorPlacement: function (error, element) {
                var errorId = element.attr("id");
                if (errorId === 'save_language_name') {
                    $("#language_name_error").text(error.text());
                } else if (errorId === 'save_language_code') {
                    $("#language_code_error").text(error.text());
                }
            },
            ...
            submitHandler: function (form) {
                ...
                formData.append('direction', $('#directionToggle').is(':checked') ? 'RTL' : 'LTR');
                formData.append('status', $('#statusToggle').is(':checked') ? 1 : 0);
                formData.append('is_default', $('#defaultToggle').is(':checked') ? 1 : 0);
                ...
                $.ajax({
                    ...
                    error: function (error) {
                        ...
                        if (error.status === 422) {
                            $.each(error.responseJSON.errors, function (key, val) {
                                $("#" + 'save_language_' + key).addClass("is-invalid");
    ...
    $(document).ready(function () {
        $('#editlanguageForm').validate({
            ...
            errorPlacement: function (error, element) {
                var errorId = element.attr("id");
                if (errorId == 'language_name') {
                    $("#name_error").text(error.text());
                } else if (errorId == 'language_code') {
                    $("#code_error").text(error.text());
                }
            },
            highlight: function (element) {
                $(element).addClass("is-invalid").removeClass("is-valid");
            ...
            submitHandler: function (form) {
                var languageId = $('#edit_language').data('id');

                // Create FormData object
                let formData = new FormData();
                formData.append('id', languageId);
                formData.append('name', $('#language_name').val());
                formData.append('code', $('#language_code').val());
                formData.append('direction', $('#rtl_toggle').is(':checked') ? 'RTL' : 'LTR');
                formData.append('status', $('#status_toggle').is(':checked') ? 1 : 0);
                formData.append('language_code', langCode);
                ...
                $.ajax({
                    ...
                    beforeSend: function () {
                        $("#saveLanguage").attr("disabled", true).html(
                            '<span class="spinner-border spinner-border-sm align-middle" role="status" aria-hidden="true"></span>'
                        );
                    },
                    success: function (response) {
                        $(".error-text").text("");
                        $(".form-control").removeClass("is-invalid is-valid");
                        $("#saveLanguage").removeAttr("disabled");
                        $('#saveLanguage').html($('#saveLanguage').data('save'));
                        if (response.success) {
                            toastr.success(response.message);
                            $('#edit_language').modal('hide');
                            language_table(); // Refresh the language table
                        } else {
                            toastr.error(response.message);
                        }
                    },
                    error: function (error) {
                        ...
                        if (error.status == 422) {
                            $.each(error.responseJSON.errors, function (key, val) {
                                $("#" + 'language_' + key).addClass("is-invalid");
                                $("#" + key + "_error").text(val[0]);
                            });
                        } else {
    ...
    $(document).on('click', '.make_default', function(e) {
        ...
        $.ajax({
            url: '/api/admin/languages/set-default',
            type: 'POST',
            data: {
                id: languageId,
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" ${(language.direction == 'RTL') ? 'checked' : ''} role="switch" disabled>
                            </div>
                        </td>
                         ${ 
                        ...
                        `<td>
                            <div class="form-check form-switch">
                                <input class="form-check-input make_default" data-id="${language.id}" type="checkbox" ${(language.is_default == '1') ? 'checked' : ''} role="switch">
                            </div>
                        </td>` : ''
                        }
                        <td>
                            <span class="badge ${language.status == '1' ? 'badge-soft-success' : 'badge-soft-danger'} d-inline-flex align-items-center">
                                <i class="ti ti-circle-filled fs-5 me-1"></i>${language.status == '1' ? 'Active' : 'Inactive'}
                            </span>
                        </td>
                         ${ 
                                        $('#has_permission').data('visible') == 1 ?
                            `<td>
                            ${language.id != 1 && $('#has_permission').data('edit') == 1  ?
                                `<li class="d-flex align-items-center" style="list-style: none;">
                                <a href="listwords/${language.id}"> <i class="ti ti-plus fs-20 m-2"></i></a>
                                    <a class=" edit_data "
                                    href="#"
...
if (pageValue === 'admin.invoice-settings') {
    $(document).ready(function() {
        ...
        $('#invoice_setting_form').submit(function(event) {
            ...
            formData.append('invoice_header_terms', $('#invoice_header_terms').val());
            formData.append('invoice_footer_terms', $('#invoice_footer_terms').val());
            formData.append('group_id', $('#group_id').val());
        ...
        function validateInvoiceForm() {
            ...
            if ($('#invoice_company_name').val().trim() === '') {
                $('#invoice_company_name').addClass("is-invalid");
                $('#invoice_company_name_error').text($('#invoice_company_name_error').data('empty'));
                isValid = false;
            }

            if ($('#invoice_header_terms').val().trim() === '') {
                $('#invoice_header_terms').addClass("is-invalid");
                $('#invoice_header_terms_error').text($('#invoice_header_terms_error').data('empty'));
                isValid = false;
            }

            if ($('#invoice_footer_terms').val().trim() === '') {
                $('#invoice_footer_terms').addClass("is-invalid");
                $('#invoice_footer_terms_error').text($('#invoice_footer_terms_error').data('empty'));
                isValid = false;
            }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.code === 200 && Object.keys(trans).length > 0) {
                    ...
                    // $('.how_it_work_update_btn').text(trans.Save);
                    // $('.how_it_work_text').text(trans.how_it_work);
                    // $('.page-title').text(trans.how_it_work);
                    // $('.dashboard_text').text(trans.Dashboard);
                    $('.lang_title').text(trans.available_translations);
                            ...
                            <tr>
                               ...
                               <div class="table-actions d-flex">
                                    <a class="delete-table view-transaction" href="javascript:void(0);"
                                        ...
                                        data-id="${item.id}">
                                        <i class="ti ti-eye fs-20 m-2"></i>                                    </a>
                                </div>
                            </td>
                            </tr>
                        `;
...
if (pageValue === 'admin.refund') {
    ...
    //                     const userdefaultImage = '/assets/img/user-default.png';
    //                     let customerImage = transaction.userimage && transaction.userimage !== ''
    //                         ? `/storage/profile/${transaction.userimage}`
    //                         : userdefaultImage;

    //                     let providerImage = transaction.providerimage && transaction.providerimage !== ''
    //                         ? `/storage/profile/${transaction.providerimage}`
    //                         : providerdefaultImage;
    ...
    //             if ($('#transactionList').length && !$.fn.DataTable.isDataTable('#transactionList')) {
    //                 $('#transactionList').DataTable({
    //                     ordering: true,
    //                     language: datatableLang
    //                 });
    ...
    function list_table(page) {
        $.ajax({
            ...
            success: function (response) {
                if (response.code == '200') {
                    listTable(response.data.transactions, response.meta);
                } else {
                    toastr.error(response.message);
                }
            },
