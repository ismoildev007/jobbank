...
function updateAuthProviderId(callback) {
    $.ajax({
        ...
        error: function () {
            if (languageId === 2) {
                loadJsonFile('unable_fetch_session', function (langtst) {
...
updateAuthProviderId(function (updatedUserId) {
    ...
    const auth_user_id = updatedUserId;

    // Call notification count
    notificationcount(auth_user_id);

    // Bind click events
    $('.notify-link').off('click').on('click', function () {
        notificationList(auth_user_id);
    });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (document.cookie.split('; ').find(row => row.startsWith('cookie_consent='))?.split('=')[1] === '1') {
...
$(document).ready(function() {
    $.ajax({
        ...
        method: 'GET',
        success: function(response) {
            let login = response.authenticated ? 1 : 0; // Set the variable here

            if (login === 1) {
                requestPermissionAndGetToken(); // Call the function directly here
            }
        },
        error: function() {
...
function renderMenus(menus) {
    menus.forEach(menu => {
        ...
        if (menu.submenus.length == 0) {
            ...
            $('.list-menus').append(`
                <li class="has-submenu" data-id="${menu.id}">
                    <a href="javascript:void(0);">${menu.name} <i class="fas fa-chevron-down"></i></a>
                    <ul class="submenu list-submenu">

                    </ul>
                </li>
            `);
            renderSubmenus(menu.id, menu.submenus);
        }
    });
}

function renderSubmenus(menuId, submenus) {
    submenus.forEach(submenu => {
        let parentItem = $(`[data-id="${menuId}"]`);
        let nestedList = parentItem.children("ul.list-submenu");

        if (submenu.submenus && submenu.submenus.length > 0) {
            nestedList.append(`
                <li class="has-submenu" data-id="${submenu.id}">
                    <a href="javascript:void(0);">${submenu.name}</a>
                    <ul class="submenu list-submenu"></ul>
                </li>
            `);
...
async function getUserId() {
    try {
        ...
        console.error('Unable to fetch session data. Please try again.');
    }
}
        ...
        $('#userProfileForm').validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    ...
                    cache: false,
                    beforeSend: function () {
                        $("#saveProfile").attr("disabled", true).html(
                            '<span class="spinner-border spinner-border-sm align-middle" role="status" aria-hidden="true"></span>'
                        );
                    },
                    success: function (response) {
                        $(".error-text").text("");
                        $("#saveProfile").removeAttr("disabled").html($('#saveProfile').data('save'));
                        $(".form-control").removeClass("is-invalid is-valid");
        ...
        try {
            ...
            if (response.code === 200) {
                ...
                if (userData.first_name != null && userData.last_name != null) {
                    $('.headerName').text(userData.first_name + ' ' + userData.last_name);
                }
            }
        } catch (error) {
            console.error('Error fetching user details:', error.responseJSON?.message || error.statusText);
        }
    }
...
if (pageValue === 'user.leads') {
    ...
    $(document).ready(function () {
        if ($('#activeStatusInput').length === 0) {
            $('body').append('<input type="hidden" id="activeStatusInput" value="" />');
        }
    });
    ...
    function loadLeads(page = 1) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.data && response.data.user_form_inputs.data.length > 0) {
                    ...
                    response.data.user_form_inputs.data.forEach((item) => {
                        ...
                        const providers = item.provider_forms_inputs.map(input => {
                                    ...
                                    <div class="col-md-8">
                                        <div class="input-block todo-inbox-check d-flex align-items-center w-50">
                                            ...
                                            <div class="strike-info">
                                                ...
                                                <p class="d-flex align-items-center custom-paragraph"><i class="ti ti-calendar me-1"></i>${item.formatted_created_at}</p>
                                            </div>
                                        </div>
                                        </div>
                                        <div class="col-md-4">
                    ...
                    const currentPage = response.data.user_form_inputs.current_page;
                        ...
                        <nav>
                            <ul class="pagination">
                                ${response.data.user_form_inputs.prev_page_url ?
                                    `<li class="page-item"><a class="page-link page_link" href="#" onclick="loadLeads(${currentPage - 1})">Previous</a></li>` : ''}
                                ...
                                ${response.data.user_form_inputs.next_page_url ?
                                    `<li class="page-item"><a class="page-link page_link" href="#" onclick="loadLeads(${currentPage + 1})">Next</a></li>` : ''}
                            </ul>
                        </nav>
                    `;
...
if (pageValue === 'user.leadsinfo') {
    ...
    $(document).on('click', '.chattab', function () {
        ...
        setSessionValue('chatproviderid',providerId,authuserid);
        setSessionValue('chatprovidername',providerName,authuserid);
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.data) {
                    ...
                    response.data.user_form_inputs.data.forEach(function(item) {
                        item.provider_forms_inputs.forEach(function(formInput,index) {
                            ...
                            if (languageId === 2) {
                                ...
                                loadJsonFile('Start Date', function (langtst) {
                                    $('.start_date').text(langtst);
                                });
                                loadJsonFile('Description', function (langtst) {
                                    $('.description').text(langtst);
                                });
                                loadJsonFile('Order Confirmed', function (langtst) {
                                    ...
                                    <div class="card-body p-3 pb-0">
                                        <div class="d-flex align-items-center justify-content-between flex-wrap">
                                            <div class="input-block todo-inbox-check d-flex align-items-center w-50 mb-3">
                                               ...
                                               <div class="strike-info">
                                                    <h4 class="mb-1">${capitalizeFirstLetter(formInput.provider.name) || 'N/A'}</h4>
                                                </div>
                                                <div class="strike-info mx-2">
                                                    <span class="badge badge-soft-warning ms-1">${capitalizeFirstLetter(item.category.name) || 'N/A'}</span>
                                                </div>
                                            </div>
                                        ...
                                        <div class="provider-quote-details p-3 border rounded bg-light" style="display: none;">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <div class="mb-2">
                                                        <strong class="text-primary quote ">Quote:</strong>
                                                        <span>${symbol || ''}</span><span>${formInput.quote || 'N/A'}</span>
                                                    </div>
                                                    <div class="mb-2">
                                                        <strong class="text-primary start_date">Start Date:</strong>
                                                        <span>
                                                        ${formInput.start_date ? new Date(formInput.start_date).toLocaleDateString('en-GB') : 'N/A'}
                                                        </span>
                                                    </div>
                                                    <div class="mb-2">
                                                ...
                                                <div class="col-md-6">
                                                    ${formInput.status === 2 && item.status === 1 && formInput.user_status != 3 && formInput.quote ?
                                                    ...
                                                    ${formInput.status === 2 && item.status === 2 && formInput.user_status === 2 && formInput.quote && item.payment_success === 1  ?
                                                        `<div class="mt-3 d-flex flex-column align-items-end">
                                                            <button class="btn btn-primary mb-2 chat-btn chattab"
                                                                    data-providerid="${formInput.provider.id}"
                                                                    data-providername="${formInput.provider.name}"
                                                                    data-userid="${item.user_id}">
                                                                Chat
                                                            </button>
                                                            <span class="text-primary" >${formInput.provider.email || 'N/A'}</span>
                                                            <span class="text-primary" >${formInput.provider.phone_number || 'N/A'}</span>
                                                        </div>
                                                        ` : ''}
                                                        ${formInput.status === 2 && item.status === 2 && formInput.user_status === 2 && formInput.quote ?
                                                            (item.payment_success === 1
                                                                ? `<div class="mt-3 d-flex justify-content-end">
                    ...
                    ['#new-tab-content', '#accepted-tab-content', '#rejected-tab-content'].forEach(function(tabId) {
                        if ($(tabId).children().length === 0) {
                            $(tabId).html('<p class="text-muted provider_list text-center mt-3">No providers found</p>');
                        }
                    });
                    ...
                    $('.btn-accept').on('click', function () {
                        ...
                        const provider_email = button.data('provider_email');
                        ...
                        $.ajax({
                            ...
                            beforeSend: function () {
                                button.attr('disabled', true).html('<div class="spinner-border spinner-border-sm text-light" role="status"></div>');
                            },
                            success: function (response) {
                                button.closest('.card').find('.chat-btn').show();
                                if (response && response.message) {
                                    ...
                                    toastr.success("Leads Accept successfully."); // Fallback message
                                }
                                providerList();
                                userList();
                            },
                            ...
                            complete: function () {
                                button.removeAttr('disabled').html('Accept');
                            }
                        });
                    });
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                if (response && response.message) {
                                    ...
                                    toastr.success("Leads Reject successfully."); // Fallback message
                                }
                                userList();
                                providerList();
                            },
                            ...
                            complete: function () {
                                button.removeAttr('disabled').html('Reject');
                            }
                        });
                    });
    ...
    $(document).on('submit', '#payment', function (event) {
        ...
        if (selectedPaymentMethod === 'paypal') {
            ...
            $.ajax({
                url: '/processpayment',
                type: 'POST',
                data: { paymenttype: 1, name: username, service_amount: amount, trx_id: trxid, refid: refid, providerid: providerid, type: 'user' },
                headers: {
                    'Authorization': 'Bearer ' + localStorage.getItem('admin_token'),
                    'Accept': 'application/json',
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            ...
            $.ajax({
                url: '/molliepayment',
                type: 'POST',
                data: { paymenttype: 3, name: username, service_amount: amount, trx_id: trxid, refid: refid, providerid: providerid, type: 'user' },
                headers: {
                    'Authorization': 'Bearer ' + localStorage.getItem('admin_token'),
                    'Accept': 'application/json',
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    if (xhr.status === 422) {
                        toastr.error(xhr.responseJSON.message || "Insufficient wallet balance");
                    } else {
            ...
            $.ajax({
                url: '/payme-process',
                type: 'POST',
                data: { paymenttype: 5, name: username, service_amount: amount, trx_id: trxid, refid: refid, providerid: providerid, type: 'user' },
                headers: {
                    'Authorization': 'Bearer ' + localStorage.getItem('admin_token'),
                    'Accept': 'application/json',
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success &&response.data.user_form_inputs.data.length > 0) {
                    ...
                    $('.category').text(`Category: ${capitalizeFirstLetter(lead.category.name)}`);
                    $('.sub_category').text(`Sub Category: ${capitalizeFirstLetter(lead.sub_category?.name) ?? '-'}`);
                    if (languageId === 2) {
                    ...
                    $('.username').text(`${capitalizeFirstLetter(lead.user.name)}`);
...
if (pageValue === 'user.provider' || pageValue === 'home') {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.data && Array.isArray(response.data)) {
                ...
                if (languageId === 2) {
                    loadJsonFile('No providers found', function (langtst) {
                        $('.no_provider').text(langtst);
                    });
                }
                if (response.data.length === 0) {
                    $('#providers-container').append('<p class="text-center no_provider">No providers found</p>');
                } else {
                    response.data.forEach(function (provider) {
                        const category = provider.category_name || 'No Category';
                        ...
                        const providerCard = `
                            <div class="col-xl-3 col-md-6 provider-card" data-provider-id="${provider.provider_id}" ${isCurrentUser ? 'style="display: none;"' : ''}>
                                <div class="card">
                                   <div class="card-body">
                                        <div class="card-img card-provider-img card-img-hover mb-3 position-relative">
                                            <div class="form-check d-flex justify-content-end image-check">
                                                <input class="form-check-input provider-checkbox" type="checkbox" value="${provider.provider_id}" ${isCurrentUser ? 'disabled' : ''}>
                                            </div>
                                            <a href="/user/providerdetails" class="provider-details-link" data-provider-id="${provider.provider_id || '24'}">
                                                <img src="${provider.profile_image ? '/storage/profile/' + provider.profile_image : '/assets/img/profile-default.png'}" alt="Img">
                                            </a>
                                        </div>
                                        <div>
                                            <div class="d-flex align-items-center justify-content-between mb-2">
                                                <div>
                                                    <h5 class="d-flex align-items-center mb-1">
                                                        <a href="/user/providerdetails" style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;" class="provider-details-link" data-provider-id="${provider.provider_id || '24'}">${provider.provider_name}</a>
                                            ...
                                            <div class="rating d-flex align-items-center justify-content-between">
                                                <div class="rating-stars d-flex align-items-center">
                                                    <i class="fas fa-star ${averageRating >= 1 ? 'filled' : ''}"></i>
                                                    <i class="fas fa-star ${averageRating >= 2 ? 'filled' : ''}"></i>
                                                    <i class="fas fa-star ${averageRating >= 3 ? 'filled' : ''}"></i>
                                                    <i class="fas fa-star ${averageRating >= 4 ? 'filled' : ''}"></i>
                                                    <i class="fas fa-star ${averageRating >= 5 ? 'filled' : ''}"></i>
                                                    <span class="d-inline-block">(${totalRatings})</span>
                                                </div>
                                                <div class="request_leads">
                                                    <a href="javascript:void(0)"
                                                    class="btn btn-sm ${isCurrentUser ? 'btn-success' : 'btn-primary'} text-white provider_send_request request-leads-btn"
                                                    data-provider-id="${provider.provider_id}"
                                                    ${isCurrentUser ? 'disabled' : ''}>
                                                    ${isCurrentUser ? 'Request Sended' : 'Send Request'}
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
                ...
                function updateSendRequestButtonState() {
                    const anySelected = $('.provider-checkbox:checked').length > 0;
                    sendRequestButton.prop('disabled', !anySelected);
                }

                // Enable or disable the button when checkboxes are clicked
                $('.provider-checkbox, .select-all-checkbox').on('change', updateSendRequestButtonState);
                ...
                $('.request-leads-btn').on('click', function () {
                    ...
                    // Disable the button and update text to "Request Sended"
                    if (languageId === 2) {
                        loadJsonFile('Requested', function (langtst) {
                            button.prop('disabled', true).text(langtst).removeClass('btn-primary').addClass('btn-success disabled');
                        });
                    }else{
                        button.prop('disabled', true).text('Requested').removeClass('btn-primary').addClass('btn-success disabled');
                    }
                    // Call the function to update leads
                    updateLeads([providerId], user_id);
                });
                ...
                $('.submit-selected').on('click', function () {
                    const selectedProviderIds = $('.provider-checkbox:checked').map(function () {
                        return $(this).val();
                    }).get();
    ...
    function updateLeads(selectedProviderIds, user_id) {
        $.ajax({
            ...
            error: function (xhr) {
                if (xhr.responseJSON && xhr.responseJSON.errors) {
                    ...
                    toastr.error('Failed to send leads request. Please try again.');
                }
            }
        });
    }
...
if (pageValue === 'user.providerlist') {
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="card-img card-provider-img card-img-hover mb-3">
                                      <a href="/user/providerdetails" class="provider-details-link" data-provider-id="${provider.provider_id || ''}">
                                        <img src="${provider.profile_image ? currentHostname + '/storage/profile/' + provider.profile_image : '/assets/img/profile-default.png'}"
                                    ...
                                    <div>
                                        <div class="d-flex align-items-center justify-content-between mb-2">
                                            <div>
                                                <h5 class="d-flex align-items-center mb-1">
                                                <a href="/user/providerdetails" class="provider-details-link" data-provider-id="${provider.provider_id || ''}">${provider.provider_name || 'Unknown Provider'}</a>
                                                    <span class="text-success ms-2"><i class="fa fa-check-circle"></i></span>
                                                </h5>
    ...
    $('#searchFilterForm').on('submit', function(e) {
        ...
        $("#searchProviderBtn").attr("disabled", true).html(
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (existingUser.length === 0) {
                        var userhtml=`<li class="user-list-item mb-2" data-user-id="${touserid}" data-user="${chatuser}"  data-authuserid="${authuserid}" data-profileimage="${profileImagePath}">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.viewpaymentproof', function (e) {
            ...
            if (paymentProof) {
                ...
                if (languageId === 2) {
                    loadJsonFile('No payment proof available.', function (langtst) {
                        msg=langtst;
                        toastr.success(msg);
                    });
                }else{
        ...
        $(document).on('click', '.bookingid', function (e) {
            ...
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    console.error(xhr.responseText);
                    toastr.error('An error occurred while trying to cancel the booking.');
                }
            });
        });
        ...
        $(document).on('click','.ordercompleteprocess',function(e){
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    console.error(xhr.responseText);
                    toastr.error('An error occurred while trying to close the booking.');
                }
            });
        });
        var langCode = $('body').data('lang');
                            ...
                            fetch('/dispute/details', {
                                ...
                                .then(data => {
                                    ...
                                    if (data.exists) {
                                        ...
                                        document.getElementById('raiseDispute').style.display = 'none';
                                        document.getElementById('adminReplySection').style.display = 'block';


                                        // Populate admin reply section
                                        document.getElementById('admin_subject').value = data.subject || '';
                                        document.getElementById('admin_content').value = data.content || '';
                                        document.getElementById('admin_reply').value = data.admin_reply || 'Not replied yet';
                                    } else {
                                        // Show dispute raise form if no reply exists
                                        document.getElementById('raiseDispute').style.display = 'block';
...
if(pageValue==='user.dashboard'){
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success && response.data.total_bookings !== undefined) {
                ...
                if (bookings.length === 0) {
                    ...
                    bookings.forEach(function(booking) {
                        ...
                        var formattedDate = new Date(booking.created_at).toLocaleDateString('en-US', {
                            year: 'numeric',
                            month: 'short',
                            day: 'numeric'
                        });

                        const defaultImage = '/front/img/profiles/avatar-04.jpg';
                        const defaultImage1 = '/front/img/services/add-service-04.jpg';

                        let categoryImage = booking.category_image && booking.category_image !== 'N/A'
                            ? `/storage/${booking.category_image}`
                            : defaultImage;

                        let providerImage = booking.creator_profile_image && booking.creator_profile_image !== 'N/A'
                            ? `/storage/profile/${booking.creator_profile_image}`
                            : defaultImage;

                        let serviceImage = booking.product_image && booking.product_image !== 'N/A'
                            ...
                            <tr>
                                <td>
                                    <a href="" class="d-flex">
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div>
                                                <h6 class="fs-14">${truncateText(capitalizeFirstLetter(booking.product.source_name), 15)}</h6>
                                ...
                                <td>
                                    <a class="d-flex">
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div>
                                                <h6 class="fs-14">${truncateText(capitalizeFirstLetter(booking.creator_name), 20)}</h6>
                                                <span class="text-gray fs-14">${truncateText(booking.creator_email, 20)}</span>
                                            </div>
                                        </div>
                                    </a>
                                </td>
                        ...
                        var transactionRowHtml = `
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <span class="dash-icon-1 bg-gray d-flex justify-content-center align-items-center rounded-circle avatar avatar-lg me-2">
                                           <img src="${categoryImage}" class="rounded-circle img-fluid" alt="Img">
                                        </span>
                                        <div>
                                            <h6 class="fs-14">${truncateText(capitalizeFirstLetter(booking.category_name), 15)}</h6>
                                            <span class="text-gray fs-12">
                                ...
                                <td class="text-end">
                                    <h6>${response.data.currencySymbol}${booking.total_amount.toFixed(2)}</h6>
                                </td>
                            </tr>
                        `;
...
function notificationcount(auth_user_id){
    $.ajax({
            ...
            if (response.code== "200") {
                ...
                if(count!=0){
                    if(data!=""){
                        ...
                        data.forEach(val => {
                            const defaultimage =  '/front/img/profiles/avatar-01.jpg';
                            const profileImage ="";
...
if (pageValue === 'productdetail') {
    ...
    function addReview(formData) {
        $.ajax({
            ...
            beforeSend: function () {
                $("#save_comment_btn").attr("disabled", true).html(
                    '<span class="spinner-border spinner-border-sm align-middle" role="status" aria-hidden="true"></span>'
                );
            },
            success: function (response) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code === 200) {
                    ...
                    $('#avg_rating').text(`(${ratingCounts.avg_rating} out of 5.0)`);
                    $('#totalReviewCount').text(`Based On ${ratingCounts.total_count} Reviews`);
                    $('#total_review_count').text(`Reviews (${ratingCounts.total_count})`);

                    if (ratingCounts.avg_rating.length >= 0) {
                        const starsContainer = $('#stars_container');
                        const avgRating = ratingCounts.avg_rating;
                        starsContainer.empty();

                        for (let i = 1; i <= 5; i++) {
                            if (i <= Math.floor(avgRating)) {
                                starsContainer.append('<i class="ti ti-star-filled text-warning me-1"></i>');
                            } else if (i === Math.ceil(avgRating) && avgRating > Math.floor(avgRating)) {
                                starsContainer.append('<i class="ti ti-star-half-filled text-warning me-1"></i>');
                            } else {
                                starsContainer.append('<i class="ti ti-star text-warning me-1"></i>');
                            }
                        }
                    }
    ...
    function renderProgressBars(ratings) {
        ...
        stars.forEach(star => {
            ...
            const percentage = ((starCount / ratings.total_count) * 100).toFixed(2);
                        ...
                        <div class="review-info">
                            <div class="d-flex align-items-center justify-content-between flex-wrap">
                                <div class="d-flex align-items-center mb-2">
                                    ...
                                    <div>
                                        <h6 class="fs-16 fw-medium">${review.first_name}</h6>
            ...
            <div class="review-info reply mt-2 mb-2 p-3">
                <div class="d-flex align-items-center justify-content-between flex-wrap">
                    <div class="d-flex align-items-center mb-2">
                        ...
                        <div>
                            <h6 class="fs-16 fw-medium">${reply.first_name}</h6>
                            <div class="d-flex align-items-center flex-wrap date-info">
    ...
    $(document).on('click', '.reply_review', function() {
        ...
        if ($('#user_id').val() == '') {
            ...
            $('#reply_' + `${id}`).find('.reply_text').remove();
            replyId = 0;
        }
    });
                ...
                if (response.success && response.data && response.data.transactions) {
                    ...
                    if (transactions.length === 0) {
                        ...
                        //   let formattedDate = new Date(transaction.date).toLocaleDateString();
                            ...
                            let customerImage = transaction.customer.image && transaction.customer.image !== 'N/A'
                                ? `/storage/profile/${transaction.customer.image}`
                                : defaultImage;

                            let providerImage = transaction.provider.image && transaction.provider.image !== 'N/A'
                                ? `/storage/profile/${transaction.provider.image}`
                                : defaultImage;
                    ...
                    $(document).on('click', '.view-transaction', function() {
                        ...
                        if (paymentType === 'Bank Transfer' && paymentStatus !== 'Paid') {
...
if (pageValue === 'productlists' || pageValue === 'productlistcategory') {
    ...
    $('#filterForm').on('submit', function () {
        $("#searchServiceBtn").attr("disabled", true).html(
            '<span class="spinner-border spinner-border-sm align-middle" role="status" aria-hidden="true"></span> Searching..'
        );
    });
    ...
    $(document).ready(function() {
        ...
        if (selectedPrice) {
            ...
            $(".filter-range-amount span").text("" + fromValue + " - " + toValue);
        }
        ...
        $("#range").ionRangeSlider({
            ...
            onChange: function (data) {
                $(".filter-range-amount span").text(" " + data.from + " - " + data.to);
            }
        });
    ...
    $('.filter_category').on('change', function() {
        if (!$(this).prop('checked')) {
            $('#all_categories').prop('checked', false);
        } else if ($('.filter_category:checked').length === $('.filter_category').length) {
            $('#all_categories').prop('checked', true);
        }
        // listSubcategory(categoryId);
        var categoryId = getCategoryIds();
            ...
            success: function (response) {
                ...
                if (response.length != 0) {
                    response.forEach((item) => {
                        $('#subcategory').append(`<option value="${item.id}" ${(item.id == selectedSubcategory) ? 'selected' : ''}>${item.name}</option>`);
                    });
                }
            },
            error: function (error) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.code === 200) {
                ...
                <div class="card shadow-none rounded">
                    <div class="card-body text-center px-2">
                        ...
                        <h6 class="mb-2">${user.user_details.company_name || 'N/A'}</h6>
                        <p class="d-flex align-items-center justify-content-center fs-14">
                            <i class="ti ti-map-pin me-1"></i>${user.user_details.company_address || 'Address Not Available'}
                        </p>
                    </div>
                </div>
            `;
            ...
            $(".our-branches-slider").append(companyCard);
                ...
                $("h5 a").text(`${user.user_details.first_name} ${user.user_details.last_name || ''}`);
                const maskedEmail = user.email.replace(/^(.)(.*)(@.*)$/, (match, p1, p2, p3) => {
                    return `${p1}xxx${p3}`;
                });
                ...
                $(".provider-bio-info h6:contains('Email')").next().text(maskedEmail);
                $(".provider-bio-info h6:contains('Phone Number')").next().text(maskedPhoneNumber);
                $(".provider-bio-info h6:contains('Language Known')").next().text(user.user_details.language);
                $(".provider-bio-info h6:contains('Address')").next().text(user.user_details.address);
                ...
                if (response.data.products.length === 0) {
                    $(".product_details").html('<p class="text-center text-muted">No products found.</p>');
                } else {
                    response.data.products.forEach(function (product) {
                        ...
                        for (let i = 0; i < 5; i++) {
                            if (i < Math.floor(averageRating)) {
                                ratingStars += '<i class="fa fa-star filled"></i>';
                            } else if (i < Math.ceil(averageRating)) {
                                ratingStars += '<i class="fa-solid fa-star-half-stroke filled"></i>';
                            } else {
                                ratingStars += '<i class="fa fa-star"></i>';
                            }
                        }
                        const defaultImage1 = '/front/img/services/add-service-04.jpg';
                        let serviceImage = product.image_url && product.image_url !== 'N/A'
                ...
                if (highestRatedProduct) {
                    const highestRatingStars = Array.from({ length: 5 }, (_, i) => {
                        return i < Math.floor(highestRatedProduct.average_rating)
                            ? '<i class="fa fa-star filled"></i>'
                            : i < Math.ceil(highestRatedProduct.average_rating)
                ...
                toastr.success("Provider details updated successfully.");
            } else {
                toastr.error(response.message || "Unable to fetch provider details.");
            }
        },
        error: function () {
            $("#pageLoader1").hide();
...
if (pageValue === 'blog-details') {
    ...
    function saveComment(formData) {
        $.ajax({
            ...
            data: formData,
            beforeSend: function () {
                $("#blogCommentBtn").attr("disabled", true).html(
                    '<span class="spinner-border spinner-border-sm align-middle" role="status" aria-hidden="true"></span>'
                );
            },
            success: function (response) {
                $(".error-text").text("");
                $("#blogCommentBtn").removeAttr("disabled").html('Post Comment');
                $(".form-control").removeClass("is-invalid is-valid");
                if (response.code === 200) {
                    toastr.success(response.message);
    ...
    $('#contactForm').validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                ...
                dataType: 'json',
                beforeSend: function () {
                    $("#contactSaveBtn").attr("disabled", true).html(
                ...
                success: function (response) {
                    $(".error-text").text("");
                    $(".form-control").removeClass("is-invalid is-valid");
                    $("#contactSaveBtn").removeAttr("disabled").html('Send Message<i class="feather-arrow-right-circle ms-2"></i>');
                    if (response.code === 200) {
                        toastr.success(response.message);
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $('.wallet_balance .balance').text(response.totalAmountBalance);
                    $('.total_debit .currency').text(response.Currency);
                    $('.total_amount .currency').text(response.Currency);
                    $('.total_debit .totalAmountdebit').text(response.totalAmountdebit);
                    ...
                    if (response.data && response.data.length > 0) {
                        // Populate table rows with wallet history data
                    ...
                    // Initialize DataTables with language settings based on languageId
                    if (languageId === 2) {
                        if ($('#walletHistoryTable').length && !$.fn.DataTable.isDataTable('#walletHistoryTable')) {
            ...
            success: function (response) {
                if (response && response.length > 0) {
                    ...
                    $('#paymentForm').on('submit', function (e) {
                        ...
                        if (!amount || amount < 50) {
                            toastr.error('Please enter a valid amount (minimum 50)');
                            return;
                        }
                        if (!paymentMethod) {
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                //     processPayment('/processpayment', amount, 'paypal', trxId, userId);
                                ...
                                <div class="d-flex align-items-center justify-content-between flex-wrap">
                                    <div class="d-flex align-items-center flex-wrap">
                                        <span class="avatar avatar-xxl ms-2 me-2">
                                            ${
                                                response.ticketdata.profile_image
                                        ...
                                        <div class="mb-2">
                                            <div class="d-flex flex-wrap align-items-center mb-1">
                                                <h6 class="fw-semibold me-2 mb-0 text-truncate">
                                                    <a href="/user/ticket-details/${response.ticketdata.ticket_id}" class="text-decoration-none text-dark">
                                                        ${response.ticketdata.subject ? response.ticketdata.subject.substring(0, 20) + '...' : "-"}
                                                    </a>
                                                </h6>
                                                <span class="ticket-status ticketstatus${response.ticketdata.id} d-flex align-items-center fs-10 ms-2" data-status="${response.ticketdata.status}">
                                                    <i class="ti ti-circle-filled me-1"></i>${response.ticketdata.ticket_status ?? "-"}
                                                </span>
                                            </div>
                                                ...
                                                ${
                                                    response.ticketdata.assignee_id
                                                        ? `<p class="d-flex align-items-center me-3 mb-1 assigneddetails${response.ticketdata.id}">
                                                                ${
                                                                    response.ticketdata.assign_profileimage
                                                                ...
                                                                <span class="text-dark">Assigned to <span class="fw-semibold ms-1 assigneename">${response.ticketdata.assignee_name ?? "-"}</span></span>
                                                        </p>`
                                                        : ""
                                                }
                                                <p class="d-flex align-items-center mb-1 me-2 fs-10">
                                                    <i class="ti ti-calendar-bolt me-1"></i>Updated ${response.ticketdata.updated_at_relative}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="d-flex flex-column align-items-end">
                                        ...
                                        <a href="/user/ticket-details/${response.ticketdata.ticket_id}" class="fs-14 bg-primary px-2 py-1 text-light mt-1 fw-bold d-flex align-items-center me-4 rounded">
