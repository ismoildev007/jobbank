...
$(document).ready(function () {
    $('.language-select').on('click', function () {
        ...
        const url = `/languagedefault/${languageId}`; // Use template literals to include languageId in the URL
...
if (pageValue === "provider.leads") {
    ...
    $(document).on("click", ".accept_btn, .reject_btn", function (e) {
        ...
        $.ajax({
            ...
            complete: function () {
                button.removeAttr("disabled").html(status === 2 ? "Accept" : "Reject");
            },
        });
    });
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (
                                            ...
                                            <div class="strike-info">
                                             <h4 class="mb-1 custom-heading fs-16">
                                                ...
                                                <p class="d-flex align-items-center custom-paragraph">
                                                    <i class="ti ti-calendar me-1"></i>${item.formatted_created_at}
                                                </p> </div>
                    ...
                    const totalPages = response.data.provider_forms_inputs.last_page;
                    const currentPage = response.data.provider_forms_inputs.current_page;
                        ...
                        const paginationHtml = `
                            <nav>
                                <ul class="pagination justify-content-center">
                                    ${
                                        response.data.provider_forms_inputs.prev_page_url
                                            ? `<li class="page-item">
                                    ...
                                    ${
                                        response.data.provider_forms_inputs.next_page_url
                                            ? `<li class="page-item">
                                                <a class="page-link" href="#" onclick="loadLeads(${
                                                    currentPage + 1
                                                }); return false;">Next</a>
                    ...
                    $(document).ready(function () {
                        ...
                        if (languageId === 2) {
                            loadJsonFile('No leads available', function (langtst) {
...
if (pageValue === "provider.leadsinfo") {
    ...
    $(document).on("click", ".accept_btn, .reject_btn", function (e) {
        ...
        $.ajax({
            ...
            complete: function () {
                button.removeAttr("disabled").html(button.hasClass("accept_btn") ? "Accept" : "Reject");
                siblingButton.show();
            },
        });
    });
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (
                    ...
                    $(".category").text(
                        `Category: ${userFormInput.category.name.charAt(0).toUpperCase()}${userFormInput.category.name.slice(1)}`
                    );

                    $(".sub_category").text(
                        `Sub Category: ${
                            userFormInput.sub_category?.name
                                ? userFormInput.sub_category.name.charAt(0).toUpperCase() + userFormInput.sub_category.name.slice(1)
                                : "-"
                        }`
                    );
                    ...
                    if (languageId === 2) {
                        loadJsonFile("Category", function (translatedCategory) {
                            $(".category").text(
                                `${translatedCategory}: ${userFormInput.category.name.charAt(0).toUpperCase()}${userFormInput.category.name.slice(1)}`
                            );
                        });

                        loadJsonFile("Sub Category", function (translatedSubCategory) {
                            const subCategoryName = userFormInput.sub_category?.name
                                ? userFormInput.sub_category.name.charAt(0).toUpperCase() + userFormInput.sub_category.name.slice(1)
                                : "-";
                            $(".sub_category").text(`${translatedSubCategory}: ${subCategoryName}`);
                        });
                    }

                    $(".username").text(
                        `${userFormInput.user.name.charAt(0).toUpperCase()}${userFormInput.user.name.slice(1)}`
                    );
                    ...
                    function disableQuoteForm(message) {
                        $("#providerQuoteForm input, #providerQuoteForm textarea, #providerQuoteForm button").prop("disabled", true);
                        $("#providerQuoteForm").append(`<div class="alert alert-info mt-3">${message}</div>`);
                    }

                    function enableQuoteForm() {
                        $("#providerQuoteForm input, #providerQuoteForm textarea, #providerQuoteForm button").prop("disabled", false);
                    ...
                    if (userFormInput.form_inputs_details && userFormInput.form_inputs_details.length > 0) {
...
if (pageValue === "providers.chat") {
    $(document).ready(function () {
        ...
        $("#addmsgform").on("submit", function (e) {
            e.preventDefault();
            $('#message-loader').removeClass('hidden');
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.code === "200") {
                        ...
                        chatWindow.append(`
                            <div class="message ${
                                touserid === authuserid ? "incoming" : "outgoing"
                            }">
                                <p>${message1}</p>
                            </div>
                        `);
                                                    ...
                                                    <a href="javascript:void(0);" class="p-2 border rounded d-block mb-2 userlist">
                                                        <div class="d-flex align-items-center">
                                                            <div class="avatar  avatar-lg avatar-online me-2 flex-shrink-0" id="useravatar${touserid}">
                                                                <img src="${profileImagePath || defaultImagePath}"    class="rounded-circle" alt="image">
                                                            </div>
                ...
                error: function (xhr) {
                    if (languageId === 2) {
                        loadJsonFile('failed_message', function (langtst) {
        ...
        $(document).on('click', '.userdatalist', function () {
            ...
            const prfimage=$(this).data('profileimage');
                $('#chat-user-name').text(userName);
                const defaultImagePath = '/front/img/profiles/avatar-01.jpg';
                const profileImagePath = prfimage && prfimage !== 'N/A'  ? `/storage/profile/${prfimage}` : defaultImagePath;
                ...
                if (existingUser.length === 0) {
                   var userhtml=`<li class="user-list-item active mb-2" data-user-id="${userId}" data-user="${userName}"  data-authuserid="${authuserid}" data-profileimage="${profileImagePath}">
                                                <a href="javascript:void(0);" class="p-2 border rounded d-block  userlist">
                                                    <div class="d-flex align-items-center">
...
if (pageValue === "provider.profile") {
        ...
        const validationMessages = {
            en: {
                ...
                postal_code: {
                    ...
                    pattern: "Postal code can only contain letters and numbers."
                },
                currency_code: {
        ...
        $("#providerProfileForm").validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        $(".error-text").text("");
                        $("#save_provider").removeAttr("disabled").html($('#save_provider').data('save'));
    ...
    function getCountries(
        ...
        $.getJSON("/states.json", function (data) {
            ...
            // Filter by country_id and exact case-sensitive name match
            const states = data.states.filter(
                (state) =>
                    state.country_id == selectedCountry &&
                    state.name.trim() === "Tashkent"
            );
                ...
                $.each(states, function (index, state) {
                    stateSelect.append(
                        $("<option>", {
                            value: state.id,
                            text: state.name,
                            selected: true, // Always select Toskent
                        })
                    );
                });
        ...
        $.getJSON("/cities.json", function (data) {
            ...
            // Filter by state_id AND city name must be in allowedCities
            const cities = data.cities.filter(
                (city) =>
                    city.state_id == selectedState &&
                    allowedCities.includes(city.name.trim())
            );
    ...
    $(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.code=== 200) {
                        if(response.data['status_code']=='2'){
                            $('.completediv'+id).show();
                            $('.acceptdiv'+id).hide();
                            $('.bookingstatus'+id).text(response.data['status']);
                            applyBookingStatusStyles();
                ...
                error: function(xhr, status, error) {
                    ...
                    if (languageId === 2) {
                        ...
                        toastr.error('An error occurred while trying to accept the booking.');
                    }
        ...
        $(document).on('click', '#completebooking', function(e) {
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    toastr.error('An error occurred while trying to complete the booking.');
                }
            });
        });
...
if (pageValue === 'provider.staffs') {
    ...
    function getStates(selectedCountry, selectedState = null) {
        ...
        const states = cachedStates.filter(state => state.country_id == selectedCountry);
        if (states.length === 1) {
    ...
    function getCities(selectedState, selectedCity = null) {
        ...
        const cities = cachedCities.filter(city => city.state_id == selectedState);
        if (cities.length === 1) {
    ...
    $('#add_staff_btn').on('click', function() {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                $("#add_staff_btn").attr("disabled", true).html(
                    '<span class="spinner-border spinner-border-sm align-middle" role="status" aria-hidden="true"></span>'
                );
            },
            success: function (response) {
                $("#add_staff_btn").removeAttr("disabled").html('<i class="ti ti-circle-plus me-2"></i>' + $('#add_staff_btn').data('add_text'));
                if (response.code === 200) {
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $(".error-text").text("");
                        $("#staff_save_btn").removeAttr("disabled").html(lg_save);
                        $(".form-control").removeClass("is-invalid is-valid");
                        $(".select2-container").removeClass('is-invalid is-valid');
                        if (response.code === 200) {
                            toastr.success(response.message);
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $(".error-text").text("");
                        $("#staff_edit_btn").removeAttr("disabled").html(lg_save);
                        $(".form-control").removeClass("is-invalid is-valid");
                        $(".select2-container").removeClass('is-invalid is-valid');
                        if (response.code === 200) {
                            toastr.success(response.message);
            ...
            success: function (response) {
                ...
                if (response.length != 0) {
                    response.forEach((item) => {
                        $('.subcategory-list').append(`<option value="${item.id}" }>${item.name}</option>`);
                    });
                }
            },
            error: function (error) {
                               ...
                               <tr>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center align-items-center">
                                        <span class="badge ${staff.status == '1' ? 'badge-soft-success' : 'badge-soft-danger'} d-flex align-items-center">
...
if (pageValue === "provider.add.service") {
    ...
    function fetchBranch() {
        $.ajax({
            ...
            success: function (data) {
                const staffContainer = $('#staff-container'); // Assume there's a div with this ID to hold the branch details
                ...
                data.branches.forEach(branch => {
                                ...
                                <div class="card">
                                    <div class="card-header">
                                        <div class="d-flex align-items-center w-100">
                                            <!-- Branch image -->
                                            <img src="${branch.branch.branch_image || 'assets/img/branches/default-branch.jpg'}" alt="Branch Image" class="avatar avatar-rounded me-2">
                                            <div>
                                        ...
                                        <div class="staff-list">
                                            ${branch.staff.map(staff => `
                                                <div class="d-flex align-items-center mb-3">
                                                    <!-- Staff profile image -->
                                                    <img src="${staff.user.profile_image || 'assets/img/staff/default-profile.jpg'}" alt="Staff Image" class="avatar avatar-sm me-2">
                                                    <div>
                                                        <div class="fs-14 fw-semibold">${staff.user.first_name} ${staff.user.last_name}</div>
                                                        <p class="mb-0 text-muted fs-12">${staff.user.mobile_number || 'No Mobile Number'}</p>
                                                    </div>
                                                    <div class="ms-auto">
                                                        <!-- Checkbox for staff selection -->
                                                        <input type="checkbox" name="staff_select[]" value="${staff.user.user_id}" class="staff-checkbox" data-branch-id="${branch.branch.id}" disabled>
                                                    </div>
                                                </div>
                                            `).join('')}
                                        </div>
                    ...
                    `;
                    staffContainer.append(branchHtml); // Append the branch details to the container
                });
    ...
    function addCheckboxLogic() {
        ...
        $('.branch-checkbox').on('change', function () {
            ...
            $(`.staff-checkbox[data-branch-id="${branchId}"]`).prop('disabled', !isChecked).prop('checked', false);
        });

        // Staff checkbox toggling
        $('.staff-checkbox').on('change', function () {
            const branchId = $(this).data('branch-id');
            const isBranchChecked = $(`.branch-checkbox[value="${branchId}"]`).is(':checked');
    ...
    $(document).ready(function() {
        ...
        $('#sendMessage').click(function () {
            ...
            if (userMessage) {
                ...
                $('#chat-box').append('<div id="' + loadingId + '"><div class="skeleton chat-skeleton label-loader mb-2"></div> <div class="skeleton chat2-skeleton label-loader"></div> </div>');
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.code === 200 && response.data) {
                            $('#' + loadingId).html('<strong>ChatGPT:</strong> <span class="chat-response">' + response.data + '</span>');
                        } else {
                            $('#' + loadingId).html('<strong>ChatGPT:</strong> Sorry, no content available.');
                        }
                    ...
                    error: function (xhr, status, error) {
                        ...
                        $('#' + loadingId).html('<strong>ChatGPT:</strong> ' + errorMessage);
                        $('#chat-container').scrollTop($('#chat-container')[0].scrollHeight);
                    },
                });
            }
    ...
    $(document).ready(function () {
        ...
        $('#service_images').on('change', function (event) {
            ...
            for (let i = 0; i < files.length; i++) {
                if (files[i].type.startsWith("image/")) {
                    ...
                    for (let j = 0; j < selectedFiles.files.length; j++) {
                        if (files[i].name === selectedFiles.files[j].name &&
                            files[i].lastModified === selectedFiles.files[j].lastModified) {
                            isDuplicate = true;
                            break;
                        }
                    }
            ...
            if (allValid) {
                ...
                errorSpan.text("Only image files are allowed. Please select valid images.");
        ...
        $priceType.change(function () {
            ...
            if (selectedValue === "hourly") {
                ...
                $minutesSection.show();
            } else if (selectedValue === "fixed" || "squre-metter" || "squre-Feet") {
                $hoursSelect.val(1);
                $hoursSection.show();
            }
        });
    ...
    $(document).ready(function () {
        $("#service-form").validate({
            ...
            messages: {
                ...
                include: {
                    ...
                    minlength:
                        "The include must be at least 3 characters.",
        ...
        $('#staff_btn').on('click', function (event) {
            ...
            if ($("#staff-form").valid()) {
                ...
                $('input[name="branch_select[]"]:checked').each(function () {
                    ...
                    const staffIds = $(`input[name="staff_select[]"][data-branch-id="${branchId}"]:checked`).map(function () {
        ...
        $("#seo-form").validate({
            ...
            messages: {
                ...
                seo_description: {
                    ...
                    maxlength: "The SEO description cannot exceed 255 characters.",
                },
            },
            errorElement: "span",
            errorPlacement: function (error, element) {
        ...
        $('#seo_btn').on('click', function (event) {
            ...
            if ($("#seo-form").valid()) {
                ...
                [...serviceFormData, ...locationFormData, ...staffFormData, ...imageFormData, ...seoFormData].forEach(function (item) {
                ...
                $("input[name='add_image[]']").each(function (index) {
                    if ($(this)[0].files.length > 0) {
                        finalFormData.append(`add_image[${index}]`, $(this)[0].files[0]);
                    }
                });
                ...
                finalFormData.append('branch_staff_payload', JSON.stringify(payload));
                ...
                $('input[name="day_checkbox[]"]:checked').each(function () {
                    ...
                    $(`input[name="start_time[${dayKey}][]"]`).each(function (index) {
                        const startTime = $(this).val();
                        const endTime = $(`input[name="end_time[${dayKey}][]"]`).eq(index).val();
            ...
            <div class="row mb-3 extra-service">
                <div class="col-xl-5">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-fill">
                            <label class="form-label">${$('#appendaddservice').data('name')} <span class="text-danger">*</span></label>
                            <input type="text" name="add_name[]" id="add_name" class="form-control" placeholder="${$('#appendaddservice').data('service_name_placeholder')}">
                ...
                <div class="col-xl-3">
                    <div class="mb-3">
                        <label class="form-label">${$('#appendaddservice').data('price')} <span class="text-danger">*</span></label>
                        <input type="number" name="add_price[]" id="add_price" class="form-control" maxlength="4" placeholder="${$('#appendaddservice').data('pricing_placeholder')}">
                        <span class="invalid-feedback" id="add_price_error"></span>
                    </div>
                </div>
                <div class="col-xl-3">
                    <div class="mb-3">
                        <label class="form-label">${$('#appendaddservice').data('description')} <span class="text-danger">*</span></label>
                        <input type="text" name="add_duration[]" id="add_duration" class="form-control" placeholder="${$('#appendaddservice').data('enter_description')}">
    ...
    $(document).on("change", ".add_image", function () {
        ...
        const preview = $(fileInput).siblings(".image-preview-wrapper").find(".img-preview");
                        ...
                        <tr>
                                ...
                                <td>
                                        <div class="form-check form-switch">
                                            <input class="form-check-input serviceId" ${(Service.status == 1) ? 'checked' : ''} type="checkbox"
                                                role="switch" id="switch-sm" data-id="${Service.id}">
                                        </div>
                                    </td>
                                ...
                                <td><li style="list-style: none;">
                                                ...
                                                <a class="delete" href="#" data-bs-toggle="modal" data-bs-target="#delete-modal" data-id="${Service.id
                    }">
...
if (pageValue === "provider.edit.service") {
    ...
    // function fetchBranch(serviceBranchIds = []) {
    //     $.ajax({
    //         url: "/api/get-staff",
    //         type: "GET",
    //         headers: {
    ...
    //             data.branches.forEach(branch => {
    //                 // Check if the current branch ID is in the array of ServiceBranchIds
    //                 const isChecked = serviceBranchIds.includes(branch.branch.id) ? 'checked' : '';
    ...
    //                                         <div>
    //                                             <div class="fs-15 fw-semibold">${branch.branch.branch_name}</div>
    //                                             <p class="mb-0 text-muted fs-11">${branch.branch.branch_address}, ${branch.branch.branch_city}, ${branch.branch.branch_state}, ${branch.branch.branch_country} - ${branch.branch.branch_zip}</p>
    //                                         </div>
    //                                         <div class="ms-auto">
    //                                             <!-- Checkbox with branch_id -->
    //                                             <input type="checkbox" name="branch_select[]" value="${branch.branch.id}" ${isChecked}>
    //                                         </div>
    //                                     </div>
    //                                 </div>
    //                             </div>
    ...
    function fetchBranch(serviceBranchIds = [], serviceBranchStaffIds = []) {
        $.ajax({
            ...
            success: function (data) {
                ...
                data.branches.forEach(branch => {
                    // Check if the current branch ID exists in serviceBranchIds
                    const isChecked = serviceBranchIds.includes(branch.branch.id) ? 'checked' : '';
                            ...
                            <div class="col-md-12">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <div class="staff-list">
                                            ${branch.staff.map(staff => {
                                                // Check if the current staff ID exists in serviceBranchStaffIds
                                                const isCheckedStaff = serviceBranchStaffIds.includes(staff.user.user_id) ? 'checked' : '';
                                                return `
                                                    <div class="d-flex align-items-center mb-3">
                                                        <!-- Staff profile image -->
                                                        <img src="${staff.user.profile_image || 'assets/img/staff/default-profile.jpg'}" alt="Staff Image" class="avatar avatar-sm me-2">
                                                        ...
                                                        <div class="ms-auto">
                                                            <!-- Checkbox for staff selection -->
                                                            <input type="checkbox" name="staff_select[]" value="${staff.user.user_id}" class="staff-checkbox" data-branch-id="${branch.branch.id}" ${isCheckedStaff}>
                                                        </div>
                                                    </div>
                                                `;
                                            }).join('')}
                ...
                // Add event listeners to control checkbox behavior
                addCheckboxLogic();
            },
            error: function (xhr) {
                const errorMessage = xhr.responseJSON && xhr.responseJSON.error
    ...
    function fetchSubcategories(languageId, categoryId, selectedSubcategory = null) {
        $.ajax({
            ...
            success: function (data) {
                let subcategoriesHtml =
                    '<option value="">Select Sub Category</option>';
                data.forEach((subcategory) => {
                    subcategoriesHtml += `<option value="${subcategory.id}" ${subcategory.id == selectedSubcategory ? "selected" : ""
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                data.forEach((category) => {
                    categoryProviderSelect += `<option value="${category.id}" ${category.id == selectedCategory ? "selected" : ""
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code === 200) {
                    ...
                    const serviceBranchesStaffs = serviceBranches.flatMap(branch => branch.staff_details);

                    const serviceBranchIds = serviceBranches.map(branch => branch.id);
                    const serviceBranchStaffIds = serviceBranchesStaffs.map(staff => staff.id);
                    ...
                    if (data.source_description && data.source_description.trim() !== "") {
                        $("#description").summernote("code", data.source_description);
                    }
                    ...
                    if (data.price_type === "free") {
                        if (priceTypeSelect.find("option[value='free']").length === 0) {
                            priceTypeSelect.append('<option value="free">Free</option>');
                        }
                    ...
                    var tagsArray = data.tags
                        .split(",")
                        .map((tag) => tag.trim());
                    tagsArray.forEach((tag) => $("#seo_tag").tagsinput("add", tag));
                    $("#include").tagsinput("removeAll");
                    var tagsArray = data.include
                        .split(",")
                        .map((include) => include.trim());
                    tagsArray.forEach((include) => $("#include").tagsinput("add", include));
                    ...
                    metadata.forEach((meta) => {
                        ...
                        switch (sourceKey) {
                            ...
                            case "premium_price_description":
                                $("#premium_price_description").val(sourceValue);
                                break;
                    ...
                    if (data.price_type === "hourly") {
                        ...
                        $(".hours-section").hide();
                    } else if (data.price_type === "fixed" || "squre-metter" || "squre-Feet") {
                        $(".hours-section").show();
                        $(".minutes-section").hide();
                    } else {
                        $(".hours-section").hide();
                    ...
                    daysOfWeek.forEach((day) => {
                        ...
                        const dayMeta = metadata.filter((meta) =>
                            meta.source_key.toLowerCase().startsWith(dayKeyPrefix)
                        );

                        if (dayMeta.length > 0) {
                            $(`#${day.toLowerCase()}_checkbox`).prop("checked", true);
                            const slotContainer = $(`#${day.toLowerCase()}Data #slotinputs`);
                            slotContainer.show();
                            slotContainer.empty();

                            dayMeta.forEach((slot, index) => {
                                const [startTime, endTime] = slot.source_Values.split(" - ");
                    ...
                    response.data.additional_services.forEach(function (service) {
                        const serviceHtml = `
                            <div class="row mb-3 extra-service">
...
if(pageValue === 'provider.subscription'){
    ...
    function renderSubscriptionCards(data) {
        ...
        data.forEach(plan => {
          ...
          // Add card content
            var innerHTML = '';
          if(plan.subscribedstatus==1 && plan.subscription_type=='topup'){
            ...
            if(plan.package_duration){
               innerHTML += `<li class="justify-content-start"><i class="fa fa-check"></i>${plan.package_duration ?? '0'} ${plan.package_term ?? '0'} / Duration</li>`;
            }
            if(plan.number_of_product){
               innerHTML += `<li class="justify-content-start"><i class="fa fa-check"></i>${plan.number_of_product ?? '0'} / No of Product</li>`;
            }
            if(plan.number_of_service_order){
               innerHTML += `<li class="justify-content-start"><i class="fa fa-check"></i>${plan.number_of_service_order ?? '0'} / No of Service Order</li>`;
            }
            if(plan.number_of_service){
               innerHTML += `<li class="justify-content-start"><i class="fa fa-check"></i>${plan.number_of_service ?? '0'} / No of Service</li>`;
            }
            if(plan.description){
    ...
    $(document).ready(function () {
        $(document).on('click','.subscribe',function(){
            ...
            $.ajax({
                ...
                type: 'POST',
                data : {amount : amt,package_id:planid,subscribetype:subscribetype, user_id: userId},
                headers: {
                    'Authorization': 'Bearer ' + localStorage.getItem('admin_token'),
                    'Accept': 'application/json',
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response && response.length > 0) {
                            ...
                            paymentmodal.empty().append(`<p>There are no payment methods available.</p>`);
                        }
                    }
                });
            }else{
                $.ajax({
                    ...
                    success: function(response) {
                        showLoader();
                        if(response!=''){
                            var msg="Subscribed Package Successfully";
                            if (languageId === 2) {
                                loadJsonFile(msg, function (langtst) {
                                    msg=langtst;
                                    toastr.success(msg);
                            ...
                            const butdiv = document.getElementById("subscribebutton"+planid);
                          var innerHTML = `<button type="button" class="btn btn-primary subscribed w-100" > Subscribed </button>`;
                          butdiv.innerHTML = innerHTML;
                        }
                    }
                });
            }
        ...
        $(document).on('submit', '#payment', function (event) {
            ...
            if (selectedPaymentMethod === 'paypal') {
                ...
                $.ajax({
                    url: '/processpayment',
                    type: 'POST',
                    data : {name: name,service_amount : serviceamount,trx_id:trxid},
                    headers: {
                        'Authorization': 'Bearer ' + localStorage.getItem('admin_token'),
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                event.preventDefault();
                $.ajax({
                    url: '/api/payme-provider-process',
                    type: 'POST',
                    data : {name: name, service_amount: serviceamount, trx_id:trxid, user_id: userId, package_id: packageId },
                    headers: {
                        'Authorization': 'Bearer ' + localStorage.getItem('admin_token'),
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
     ...
     function loadTabData(type) {
        ...
        const activeTab = document.getElementById(type === 'regular' ? 'regularTab' : 'topupTab');
            ...
            if (response.code== "200") {
                ...
                if(data!==null){
                    ...
                    if(data.overall_total_amount){
                        $('.totalincome').text(currencyval + data.overall_total_amount);
                    }
                    if(data.processed_amount){
                        $('.completeincome').text(currencyval + data.processed_amount);
                    }
                    if(data.remaining_amount){
                        $('.totaldue').text( currencyval + data.remaining_amount);
                    }
                }
            } else {
                toastr.error(response.message);
        ...
        success: function(response) {
            if (response.code== "200") {
                ...
                if((data!='') && data!=null){
                    ...
                    $('.planprice').text(currency+data.price);
                }else{
                    $('.popularplan').hide();
                }
            } else {
            ...
            if (response.code== "200") {
                ...
                if(data!=""){
                    response.data.forEach(val => {
                        ...
                        const defaultproductpath = '/front/img/services/add-service-04.jpg';
                        let productImage = val.productimage && val.productimage !== 'N/A'
                        ? `/storage/${val.productimage}`
                        : defaultproductpath;
                        const defaultprofileImage = '/assets/img/profile-default.png';
                         let profileImagePath = val.profile_image && val.profile_image !== 'N/A'
                        ? `/storage/profile/${val.profile_image}` : defaultprofileImage;
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.code== "200") {
                ...
                if(data!=""){
                    data.forEach(val => {
                        const defaultImagePath = '/front/img/profiles/avatar-04.jpg';
                        let userImage = val.profile_image && val.profile_image !== 'N/A'
                    ...
                    var ratehtml=` <div class=" border-bottom pb-3 mb-3">
                            <div class="d-flex justify-content-between align-items-center flex-wrap row-gap-2">
                                ...
                                <div class="d-flex">
                                    ...
                                    ratehtml+=`</span>
                                    <span class="fs-12">${val.rating ?? 0}</span>
                                </div>
                            </div>
                        </div>`;
                ...
                if (data != "") {
                    data.forEach(val => {
                        ...
                        if (val.average_rating != '') {
                            servicehtml += `<span><i class="ti ti-star-filled text-warning me-1"></i>${val.average_rating ?? 0}</span>`;
                        }
                                ...
                                <div class="d-flex">
                                    ...
                                    <div>
                                        ...
                                        if (languageId === 2) {
                                           ...
                                           if(val.average_rating!=''){
                                                servicehtml +=`<span><i class="ti ti-star-filled text-warning me-1 me-1"></i>${val.average_rating ?? 0}</span>`;
                                            }
                       servicehtml +=`  </div>
                                    </div>
                                </div>
...
function notificationcount(auth_provider_id){
    $.ajax({
...
if(pageValue=== 'provider.transaction'){
    ...
    $('#uploadPaymentProof').on('click', function () {
        ...
        if (!currentBookingId) {
            toastr.error('Booking ID is missing. Please refresh and try again.');
            return;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    toastr.success('Payment proof uploaded successfully. Status updated to Paid.');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        codFileInput.addEventListener('change', (event) => {
            ...
            if (!allowedTypes.includes(file.type)) {
                toastr.error('{{ __("Invalid file type. Only images and PDFs are allowed.") }}', '{{ __("Error") }}');
                codFileInput.value = ''; // Clear the input
                return;
            }

            if (file.size > maxSize) {
                toastr.error('{{ __("File size exceeds the maximum limit of 2MB.") }}', '{{ __("Error") }}');
                codFileInput.value = ''; // Clear the input
                return;
            }
            ...
            if (file.type.startsWith('image/')) {
                ...
                pdfPreview.textContent = '{{ __("PDF file selected: ") }}' + file.name;
    ...
    function bookingTransactionList(){
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success && response.data && response.data.transactions) {
                    ...
                    if (transactions.length === 0) {
                        ...
                        tableBody += `
                            <tr>
                                <td colspan="11" class="text-center">${$('#transactionList').data('empty')}</td>
                            </tr>`;
                    } else {
                        ...
                        let formattedDate = transaction.date;
                            ...
                            const defaultImage = '/assets/img/profile-default.png';
                            const defaultImage1 = '/front/img/services/add-service-04.jpg';
                            ...
                            let serviceImage = transaction.service.service_image && transaction.service.service_image !== 'N/A'
                                ? `/storage/${transaction.service.service_image}`
                                : defaultImage1;
                            ...
                            tableBody += `
                                <tr>
                                    ...
                                    <td>${currency}${transaction.amount.commission}</td>
                                    <td>${currency}${transaction.amount.final_total}</td>
                                    <td>${formattedDate}</td>
                                    <td>${paymentType}</td>
                                    ...
                                    <td>
                                        <div class="table-actions d-flex">
                                            ...
                                            data-payment-proof="${transaction.payment.payment_proof}"
                                            data-transaction_id="${transaction.payment.transaction_id}"
                    ...
                    $(document).on('click', '.view-transaction', function() {
                        ...
                        if (paymentProof) {
                            ...
                            const fileExtension = paymentProof.split('.').pop().toLowerCase();
                            if (['jpg', 'jpeg', 'png', 'gif'].includes(fileExtension)) {
                                filePreview.html(`<img src="${window.location.origin}/storage/${paymentProof}" alt="Payment Proof" class="img-fluid rounded">`);
                                ...
                                filePreview.html(`<p class="text-danger">Unsupported file type</p>`);
                            }
                        } else {
            ...
            error: function() {
                toastr.error('Error while retrieving transactions. Please try again.');
            }
        });
    }
...
if (pageValue === 'provider.payouts') {
    ...
    function setupValidation(payoutType) {
        ...
        if (payoutType == 1) {
            ...
            messages = {
                holder_name: { required: $('#holder_name_error').data('required') },
                bank_name: { required: $('#bank_name_error').data('required') },
                account_number: { required: $('#account_number_error').data('required') },
    ...
    function savePayout(formData) {
        $.ajax({
            ...
            data: formData,
            beforeSend: function () {
                $("#payout_save_btn").attr("disabled", true).html(
                    '<span class="spinner-border spinner-border-sm align-middle" role="status" aria-hidden="true"></span>'
                );
            },
            success: function (response) {
                $(".error-text").text("");
                $("#payout_save_btn").removeAttr("disabled").html($('#payout_save_btn').data('save'));
                $(".form-control").removeClass("is-invalid is-valid");
            ...
            success: function (response) {
                if (response.code === 200) {
                    ...
                    if (payoutHistory.length === 0) {
                        ...
                        tableBody += `
                            <tr>
                                <td colspan="8" class="text-center">${$('#payoutHistoryTable').data('empty')}</td>
                            </tr>`;
                    } else {
                        payoutHistory.forEach((payout, index) => {
                            tableBody += `
                                <tr>
                                    ...
                                    <td>
                                        <div class="">
                                            <a href="${payout.payment_proof_path}" download="PaymentProof">
                    ...
                    if ((payoutHistory.length != 0) && !$.fn.DataTable.isDataTable('#payoutHistoryTable')) {
    ...
    $("#requestForm").validate({
        ...
        submitHandler: function (form) {
            $.ajax({
                ...
                beforeSend: function () {
                    $("#requestSendBtn").attr("disabled", true).html(
                        '<span class="spinner-border spinner-border-sm align-middle" role="status" aria-hidden="true"></span>'
                    );
                },
                success: function (response) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code === 200) {
                    ...
                    $('.available_amount').text(provider.currency_symbol + provider.available_amount);
                    $('#last_payout').text(provider.currency_symbol + provider.last_payout);
...
if(pageValue==='providerpayment.success' || pageValue==='provider.subscriptionsuccess'){
    const container = document.getElementById("subscriptionCards");
    container.innerHTML = "";
...
$("#deleteAccountForm").validate({
    ...
    submitHandler: function (form) {
        $.ajax({
            ...
            dataType: 'json',
            beforeSend: function () {
                $("#deleteAccountBtn").attr("disabled", true).html(
                    '<span class="spinner-border spinner-border-sm align-middle" role="status" aria-hidden="true"></span>'
                );
            },
            success: function (response) {
                $(".error-text").text("");
                $("#deleteAccountBtn").removeAttr("disabled").html($('#deleteAccountBtn').data('delete'));
                $(".form-control").removeClass("is-invalid is-valid");
            ...
            list['historydata'].forEach(data => {
                ...
                tableBody += `
                    <tr>
                       ...
                       <td><span class="payment-status fs-14" data-status="${data.payment_status}">${data.paymentstatus}</spn></td>
                        <td><span class="active-status fs-14" data-status="${data.status}">${data.activestatus}</spn></td>`;
                       tableBody += `
                            <td>  <a href="#"
...
if (pageValue === 'provider.reviews') {
                ...
                if (response.code === 200) {
                    ...
                    if (reviews.length > 0) {
                        reviews.forEach(review => {
                            ...
                            for (let i = 1; i <= 5; i++) {
                                if (i <= review.rating) {
                                    filledStars += '<span><i class="ti ti-star-filled text-warning"></i></span>';
                                } else {
                                    filledStars += '<span><i class="ti ti-star text-muted"></i></span>';
                                }
                            }
                                    ...
                                    <div class="card shadow-none">
                                        <div class="card-body">
                                            <div class="d-md-flex align-items-center">
                                                ...
                                                ${
                                                    $('#has_permission').data('del_permission') == 1 ?
                                                    `<div class="user-icon d-inline-flex">
                            ...
                            <div class="col-xxl-12 col-lg-12">
                                <div class="card shadow-none">
                                    <div class="card-body d-flex align-items-center justify-content-center">
                                        <span class="text-center text-black">${$('.review_list_container').data('empty_info')}</span>
                                    </div>
                                </div>
                            </div>
                        `);
                    ...
                    const endRecord = currentPage * perPage > totalReviews ? totalReviews : currentPage * perPage;
                    $('#pagination_info').text(`${(currentPage - 1) * perPage + 1} - ${endRecord} of ${totalReviews}`);

                    let paginationLinks = '';
                    for (let i = 1; i <= lastPage; i++) {
                        paginationLinks += `
                            <li class="page-item me-2">
                                <a class="page-link-1 ${i === currentPage ? 'active' : ''}" href="javascript:void(0);" onclick="getProviderReviews(${i});">${i}</a>
                            </li>
                        `;
                    }
    ...
    $(document).on('click', '#deleteReviewConfirm', function(e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                $("#deleteReviewConfirm").removeAttr("disabled").html($('#deleteReviewConfirm').data('yes'));
        ...
        const validationMessages = {
            en: {
                    ...
                    new_password: {
                        ...
                        minlength: "Password must be at least 8 characters long.",
                        notEqualTo: "New password cannot be the same as the current password."
                    },
                    confirm_password: {
                        required: "Confirm password is required.",
                        equalTo: "The confirmation password doesn't match the new password."
                    }
            },
            ar: {
                    current_password: {
                    ...
                    new_password: {
                        ...
                        minlength: "يجب أن تكون كلمة المرور 8 أحرف على الأقل.",
                        notEqualTo: "لا يمكن أن تكون كلمة المرور الجديدة هي نفسها كلمة المرور الحالية."
                    },
                    confirm_password: {
                        required: "تأكيد كلمة المرور مطلوب.",
                        equalTo: "كلمة المرور التأكيدية لا تطابق كلمة المرور الجديدة."
                    }
            },
        };
...
if (pageValue === 'provider.addbranch') {
    ...
    function saveBranch() {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                $("#staff_btn").attr("disabled", true).html(
                    '<span class="spinner-border spinner-border-sm align-middle" role="status" aria-hidden="true"></span>'
                );
            },
            success: function (response) {
                $(".error-text").text("");
                $("#staff_btn").removeAttr("disabled").html($('#staff_btn').data('save'));
                $(".form-control").removeClass("is-invalid is-valid");
                $(".select2-container").removeClass('is-invalid is-valid');
                if (response.code === 200) {
                    toastr.success(response.message);
            ...
            success: function (response) {
                if (response && response.data && response.data.length > 0) {
                    ...
                    response.data.forEach(city => {
                        stateDropdown.append(`<option value="${city.id}">${city.name}</option>`);
                    });
                }
            },
            error: function (error) {
...
if (pageValue === 'provider.branch') {
    ...
    $(document).on('click', '#addBranchBtn', function () {
        $.ajax({
            ...
            success: function (response) {
                $("#addBranchBtn").removeAttr("disabled").html($('#addBranchBtn').data('add_text'));
                if (response.code === 200) {
                    if (response.no_package === true) {
                        $("#no_sub").modal("show");
                    }
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code === 200) {
                    ...
                    if ((staffs.length != 0) && !$.fn.DataTable.isDataTable('#branchTable')) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response && response.data && response.data.length > 0) {
                    const stateDropdown = $('#state');
                    response.data.forEach(state => {
                        stateDropdown.append(`<option value="${state.id}" ${state.id == $('#state').data('state') ? 'selected' : ''}>${state.name}</option>`);
                    });
                }
            },
            error: function (error) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response && response.data && response.data.length > 0) {
                    const stateDropdown = $('#city');
                    response.data.forEach(city => {
                        stateDropdown.append(`<option value="${city.id}" ${city.id == $('#city').data('city') ? 'selected' : ''}>${city.name}</option>`);
                    });
                }
            },
            error: function (error) {
        ...
        $('#Ticketform').on('submit', function (event) {
            ...
            if(isValid==true){
                ...
                $.ajax({
                    ...
                    beforeSend: function () {
                        $(".add_ticket_btn").attr("disabled", true).html('<span class="spinner-border spinner-border-sm align-middle" role="status"></span>');
                    }
                })
                .done((response) => {
                                ...
                                <div class="d-flex align-items-center justify-content-between flex-wrap">
                                    <div class="d-flex align-items-center flex-wrap">
                                        <span class="avatar avatar-xxl me-2">
                                            ${
                                                response.ticketdata.profile_image && response.ticketdata.profile_image !== ''
                                                    ...
                                                    : response.ticketdata.user_type === 'User'
                                        ...
                                        <div class="mb-2">
                                            <div class="d-flex flex-wrap align-items-center mt-3">
                                                <h6 class="fw-semibold me-2 mb-0">
                                                    <a onclick="storeTicketId(${response.ticketdata.id})" class="text-decoration-none text-dark">
                                                        ${response.ticketdata.subject ? response.ticketdata.subject.substring(0, 20) + '...' : "-"}
                                                    </a>
                                                </h6>
                                                <span class="ticket-status ticketstatus${response.ticketdata.id} d-flex align-items-center ms-2 fs-10" data-status="${response.ticketdata.status}">
                                                    <i class="ti ti-circle-filled fs-6 me-1"></i>${response.ticketdata.ticket_status ?? "-"}
                                                </span>
                                            </div>
                                            
                                            <div class="d-flex flex-wrap align-items-center mt-1">
                                                ${response.ticketdata.assignee_id ? `
                                                <p class="d-flex align-items-center me-3 mb-1 assigneddetails${response.ticketdata.id}">
                                                    <span class="fw-semibold text-muted me-2">Assigned to:</span>
                                                    <img src="${response.ticketdata.assign_profileimage && response.ticketdata.assign_profileimage !== '' ? response.ticketdata.assign_profileimage : '/assets/img/user-default.jpg'}" class="rounded-circle me-2" width="24" height="24" alt="img">
                                                    <span class="text-dark fw-semibold ms-1 assigneename">${response.ticketdata.assignee_name ?? "-"}</span>
                                                </p>` : ""}
                                                
                                                <p class="d-flex align-items-center mb-1 me-2 fs-10">
                                                    <i class="ti ti-calendar-bolt me-1"></i>
                                                    <span class="fw-semibold text-muted me-1">Updated:</span> ${response.ticketdata.updated_at_relative}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="d-flex flex-column align-items-end">
                                        <div class="d-flex flex-wrap align-items-center fs-10">
                                            ...
                                            <span class="badge bg-info text-light rounded-pill me-3">#${response.ticketdata.ticket_id ?? "-"}</span>
                                            ...
                                            <span class="priority-status d-inline-flex align-items-center me-4" data-status="${response.ticketdata.priority}">
                                                <i class="ti ti-circle-filled fs-6 me-1"></i>${response.ticketdata.priority ?? "-"}
                                            </span>
                                        </div>
                                        <div class="mb-3">
                                            <a href="${$('#ticket-list').data('user_type') == 2 ? `/provider/ticket-details/${response.ticketdata.ticket_id}` : `/staff/ticket_details/${response.ticketdata.ticket_id}`}" class="fs-14 bg-primary px-2 py-1 text-light mt-1 fw-bold text-dark d-flex align-items-center me-4 rounded">
                                                <i class="ti ti-eye"></i>
                                            </a>
                                        </div>
                                    </div>
...
if(pageValue==='provider.ticketdetails' || pageValue==='staff.ticket_details'){
    applyTicketStatusStyles();
    function   applyTicketStatusStyles(){
        $('.ticket-status').each(function (index) {
            const status = $(this).data('status');
